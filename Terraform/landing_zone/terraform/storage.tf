resource "azurerm_storage_account" "boot_diag" {
  name                     = ("diag${var.REGION_ABR}${var.ENV}${var.ORG}")
  location                 = var.REGION
...
#  }
 ...
 resource "azurerm_storage_account" "logs" {
  name                     = ("logs${var.REGION_ABR}${var.ENV}${var.ORG}")
  location                 = var.REGION
  resource_group_name      = azurerm_resource_group.logs.name
  account_tier             = "Standard"
  access_tier              = "Hot"
...
resource "azurerm_private_endpoint" "logs_blob_endpoint" {
   depends_on          = [ azurerm_private_dns_zone.private_dns_zone ]
   name                = "${azurerm_storage_account.logs.name}-blob-pe"
   location            = var.REGION
 ...
 resource "azurerm_private_endpoint" "logs_queue_endpoint" {
   depends_on          = [ azurerm_private_dns_zone.private_dns_zone ]
   name                = "${azurerm_storage_account.logs.name}-queue-pe"
   location            = var.REGION
   resource_group_name = azurerm_storage_account.logs.resource_group_name
   subnet_id           = azurerm_subnet.pe.id
   lifecycle {
   ...
   private_dns_zone_group {
     name                 = "privatelink.blob.core.windows.net"
     private_dns_zone_ids = ["/subscriptions/${var.SUB}/resourceGroups/${azurerm_resource_group.dns.name}/providers/Microsoft.Network/privateDnsZones/privatelink.queue.core.windows.net"]
   }
 }
